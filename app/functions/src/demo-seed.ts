...
type Phase = 'stable' | 'sick' | 'recovery' | 'declining'

function getPhase(date: string): Phase {
  if (date >= '2026-01-10' && date <= '2026-01-13') return 'sick'
  if (date >= '2026-01-14' && date <= '2026-02-10') return 'recovery'
  if (date >= '2026-03-05') return 'declining'
  return 'stable'
}
...
function buildRecord(index: number, date: string, time: string, phase: Phase): CareRecord {
  ...
  let meal: StructuredData['meal']
  let sleep: StructuredData['sleep']
  let physical: StructuredData['physical']
  let mental: StructuredData['mental']
  ...
  const elimination: StructuredData['elimination'] = {
  ...
  const medication: StructuredData['medication'] = {
  ...
  const care_given_map: Record<Phase, string[]> = {
...
export const demoRecords: CareRecord[] = generateRecords()
...
export const demoReports = [
  {
    ...
    additionalNotes: '',
    overallAssessment:
      '11月は全体的に安定した状態が続きました。食事摂取量は概ね7〜9割と良好で、体重の大きな変動も見られませんでした。血圧は130/80前後で推移し、かかりつけ医の指導範囲内でした。認知機能も比較的安定しており、日常的な会話や簡単な作業に取り組むことができました。',
    adlSummary:
      '歩行は手すりを使用しながらも自室内は概ね自立。食事は見守り・一部介助で摂取可能。排泄はほぼ自立（夜間のみ声掛け必要）。入浴は週2回、介助にて実施。更衣・整容は声掛けにより自分で行うことが多かった。',
    mentalSummary:
      '精神面は穏やかな日が多く、好きなテレビ番組を楽しむ様子が見られました。軽度の短期記憶障害は持続していますが、日常生活に大きな支障はありませんでした。夜間の不穏は月に2〜3回程度認められました。',
    incidents: '特記すべきインシデントなし。月中旬に軽い転倒リスクの場面が一度あったが、怪我には至らなかった。',
    handoverNotes:
      '引き続き現在のケアプランを継続。12月は年末年始の環境変化に伴う混乱に注意が必要。服薬管理を徹底し、血圧値のモニタリングを継続してください。',
  ...
  },
  {
    ...
    additionalNotes: '',
    overallAssessment:
      '12月は年末の環境変化にもかかわらず比較的安定して過ごせました。クリスマスや年末年始の行事に参加し、楽しそうにしている姿が見られました。食事摂取は概ね良好で、体重は前月とほぼ変わらず。ただし月末にかけて若干の睡眠リズムの乱れが見られ始めました。',
    adlSummary:
      'ADLは11月とほぼ同水準を維持。年末の来客により普段と異なる環境になることが多く、一時的に歩行の不安定さが見られた場面もありました。入浴介助は継続。食事は好物を提供すると全量摂取できる日が多くありました。',
    mentalSummary:
      '家族の来訪で表情が明るくなる場面が多く見られました。一方、慣れない場所や人物に対する混乱も数回ありました。「家に帰りたい」との発言が月末に2回ありましたが、声掛けで落ち着きました。',
    incidents:
      '12月28日、居間でのつまずきがあった。転倒には至らなかったが、床に散乱していた物への注意が必要と判断し、環境整備を実施。',
    handoverNotes:
      '1月は新年の環境変化に注意。定期受診を予定通り実施すること。睡眠リズムの乱れが継続する場合は主治医へ相談を検討してください。',
  ...
  },
  {
    ...
    additionalNotes: '',
    overallAssessment:
      '1月は月初めは安定していましたが、1月10〜13日にかけて発熱（最高38.2度）を伴う風邪を発症しました。かかりつけ医を受診し、風邪と診断され解熱剤を処方。その後14日以降は体温が平熱に戻りましたが、食欲回復に時間がかかり、月の後半まで回復期が続きました。発熱期間中は食事摂取量が著しく低下し、水分補給を優先しました。',
    adlSummary:
      '発熱期間中は安静臥床が多く、一時的にADLが低下しました。回復後も体力の回復に伴い段階的にADLが戻り、月末には概ねもとの水準に近づきました。入浴は発熱期間中は清拭に変更し、回復後に再開しました。',
    mentalSummary:
      '発熱時および回復期に認知機能の変動が見られました。時折「ここはどこ？」と繰り返す場面や、家族の顔を一時的に認識できない場面もありました。体調の回復とともに認知面も改善傾向にありますが、引き続き観察が必要です。',
    incidents:
      '1月10〜13日：発熱による体調不良、かかりつけ医受診。1月11日：発熱中の一時的な歩行困難（ベッドサイドで支えが必要）。転倒なし。',
    handoverNotes:
      '2月は感染予防を徹底すること。認知機能の変動が続く場合は主治医への相談を検討。食事摂取量の回復経過を継続観察してください。',
  ...
  },
  {
    ...
    additionalNotes: '',
    overallAssessment:
      '2月は1月の風邪からの回復が続き、月後半には食事摂取量もほぼ回復しました。ただし月全体を通して睡眠の乱れや認知機能の変動が見られ、安定した状態とは言い難い時期でした。血圧は概ね130〜140/80〜90の範囲で推移し、若干高めの日もありました。転倒・受診が必要な急変はありませんでした。',
    adlSummary:
      '1月の発熱・回復を経てADLは概ね安定しましたが、体力的な低下が若干見られました。歩行時のふらつきが月前半に時々見られたため、移動時の見守りを強化しました。食事は月後半に改善し、7〜8割の摂取が安定してきました。',
    mentalSummary:
      '認知面では月前半に混乱エピソードが複数回ありました（「今日は何日？」「息子はどこ？」など）。夜間の覚醒も続き、不眠傾向が見られました。月後半から徐々に安定し、日中の覚醒水準も上がってきました。',
    incidents:
      '特記すべき転倒・受診は発生していません。2月15日、夜間の不穏あり、30分程度声掛けにて対応。介護者への精神的負担に配慮が必要です。',
    handoverNotes:
      '3月は認知機能の変動と睡眠状態の経過観察を継続してください。食欲の変動にも注意が必要です。必要に応じてかかりつけ医への相談をご検討ください。介護者の負担軽減のため、レスパイトケアの利用も検討に値します。',
