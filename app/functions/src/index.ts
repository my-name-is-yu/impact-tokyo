...
const alerts: AlertNotification[] = [
  {
    ...
    description:
      'Appetite has been declining since March 5. There is a risk of dehydration and malnutrition. Reporting to the Care Manager is recommended.',
    severity: 'high',
    suggestion:
  ...
  },
  {
    ...
    description:
      'The Long-term Care Insurance certification for Setsuko Tanaka expires at the end of April 2026. Let\'s start preparing for the renewal process.',
    severity: 'medium',
  ...
  },
  {
    ...
    description:
      'A draft report for the Care Manager has been created based on the February 2026 care records. Please review and edit as needed.',
    severity: 'low',
    ...
    const tools: Anthropic.Tool[] = [
      {
        ...
        description:
          'Analyze recent care records to detect health anomaly patterns (e.g., declining appetite, sleep issues, cognitive changes). Call this when you want to check for concerning trends.',
        input_schema: {
      ...
      },
      {
        name: 'create_alert',
        description:
          'Create a proactive alert notification for the caregiver. Use when a concerning pattern is detected that the caregiver should know about.',
        input_schema: {
          type: 'object' as const,
          properties: {
      ...
      },
      {
        ...
        description:
          'Draft a message to send to the care manager about a health concern. Use when the situation warrants professional attention.',
        input_schema: {
          type: 'object' as const,
          properties: {
      ...
      },
      {
        ...
        description:
          'Suggest generating a monthly report if sufficient records exist. Use when it is near the end of the month or enough records have accumulated.',
        input_schema: {
          type: 'object' as const,
          properties: {
...
${JSON.stringify(recentRecords, null, 2)}`
    ...
    for (const block of response.content) {
      ...
      if (block.name === 'analyze_health_pattern') {
        ...
        const affectedAreas = input.affected_areas as string[]
        console.log(
          `[Orchestrator] Health pattern analysis — severity: ${severity}, areas: ${affectedAreas.join(', ')}, pattern: ${pattern}`,
        )
        ...
        console.log(
          `[Orchestrator] Care manager message drafted — urgency: ${urgency}, subject: ${subject}\n${body}`,
        )
        actions.push(`draft_care_manager_message: ${subject}`)
      } else if (block.name === 'suggest_report_generation') {
        const reason = input.reason as string
        const recordCount = input.record_count as number
        const alert: AlertNotification = {
          id: generateId(),
          userId: record.userId,
          careRecipientId: record.careRecipientId,
        ...
        alerts.push(alert)
        console.log(
          `[Orchestrator] Report generation suggested — ${recordCount} records available: ${reason}`,
        )
...
export const api = onRequest(
  { memory: '512MiB', timeoutSeconds: 300, region: 'asia-northeast1', secrets: ['ANTHROPIC_API_KEY'] },
  app,
)
